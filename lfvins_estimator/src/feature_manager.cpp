...
{
    ROS_INFO("input point feature: %d", (int)image.size());
    ROS_INFO("input line feature: %d", (int)lines.size());
    ROS_INFO("num of feature: %d", getFeatureCount());  // 已有的特征数目
    double parallax_sum = 0;
    int parallax_num = 0;
    last_track_num = 0;
    for (auto &id_pts : image)        //遍历当前帧上的特征
...
double FeatureManager::reprojection_error( Vector4d obs, Matrix3d Rwc, Vector3d twc, Vector6d line_w ) {
    ...
    // double l_norm = nc(0) * nc(0) + nc(1) * nc(1) +nc(2) * nc(2);
    // double l_sqrtnorm = sqrt( l_norm );
    // double l_trinorm = l_norm * l_sqrtnorm;

    

    // double l_obs_0=obs[1]-obs[3];
    // double l_obs_1=obs[2]-obs[0];
    // double l_obs_2=obs[0]*obs[3]-obs[2]*obs[1];

    // double l_obs_norm = l_obs_0* l_obs_0 + l_obs_1 * l_obs_1 +l_obs_2 * l_obs_2;
    // double l_obs_sqrtnorm = sqrt( l_obs_norm );
    // //double e1 = (l_obs_0 * nc(0) + l_obs_1* nc(1) + l_obs_2*nc(2))/l_obs_sqrtnorm;
    // error =(l_obs_0 * nc(0) + l_obs_1* nc(1) + l_obs_2*nc(2))/l_obs_sqrtnorm/l_sqrtnorm;

    // ROS_WARN("xulei error:%f",error);

    // return error;

    //shiotng 
    Vector3d vc=line_c.head(3).normalized();
    Vector3d vc2=p1.cross(p2);
    vc2=vc2.normalized();

    // ROS_WARN("shitong error:%f",error);
    // error=abs(vc.dot(vc2));
    error=vc.dot(vc2);
    if(error<0)
        error*=-1;
