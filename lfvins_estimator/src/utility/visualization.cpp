                 ...
                 Eigen::Matrix3d loop_correct_r)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        cout << estimator.drift_correct_r << ';' << estimator.drift_correct_t << endl;
                   ...
                   Eigen::Matrix3d loop_correct_r)
{
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        Vector3d w_pts_i = loop_correct_r * estimator.Rs[imu_i] * (estimator.ric[0] * pts_i + estimator.tic[0]) + loop_correct_r * estimator.Ps[imu_i] + loop_correct_t;
                   ...
                   Eigen::Matrix3d loop_correct_r)
{
    ...
    for (auto &it_per_id : estimator.f_manager.linefeature)
    {
        ...
        Vector3d w_pts_1 = loop_correct_r * estimator.Rs[imu_i] * (estimator.ric[0] * pts_1 + estimator.tic[0]) + loop_correct_r * estimator.Ps[imu_i] + loop_correct_t;
        Vector3d w_pts_2 = loop_correct_r * estimator.Rs[imu_i] * (estimator.ric[0] * pts_2 + estimator.tic[0]) + loop_correct_r * estimator.Ps[imu_i] + loop_correct_t;
    ...
    marg_lines_cloud.color.r = 0;
    marg_lines_cloud.color.g = 0;
    marg_lines_cloud.color.b = 0;
    ...
    for (auto &it_per_id : estimator.f_manager.linefeature)
    {
        ...
        if (it_per_id.start_frame == 0 && it_per_id.linefeature_per_frame.size() <= 2 && it_per_id.is_triangulation == true)
        {
            ...
            Vector3d ncnorm = nc.normalized();
            Vector3d vcnorm = vc.normalized();
            float _distance = abs((float)nc.norm() / (float)vc.norm());
            Vector3d arrow = vc.cross(nc);

            // ROS_WARN("nc:(%f,%f,%f)  vc:(%f,%f,%f)", nc[0], nc[1], nc[2], vc[0], vc[1], vc[2]);

            if (abs(vcnorm[2]) > 0.5 && _distance < 2)
                continue;
