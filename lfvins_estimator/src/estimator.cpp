...
void Estimator::setParameter()
{
    ...
    ProjectionFactor::sqrt_info = FOCAL_LENGTH / 0.5 * Matrix2d::Identity();
    lineProjectionFactor::sqrt_info = FOCAL_LENGTH / 2 * Matrix2d::Identity();
...
{
    MatrixXd lineorthRaw = f_manager.getLineOrthVector(Ps, tic, ric);
    ...
    for (int i = 0; i < f_manager.getLineFeatureCount(); ++i)
    {
        Vector4d orthRaw = lineorthRaw.row(i);
        Vector4d orth(para_LineFeature[i][0],
                      para_LineFeature[i][1],
                      para_LineFeature[i][2],
                      para_LineFeature[i][3]);

        Vector3d directRaw = orthRaw.head(3).normalized();
        Vector3d direct = orth.head(3).normalized();

        double errorAngle = abs(directRaw.dot(direct));
        // ROS_WARN("error angle : %f ",errorAngle);
        // if(errorAngle>0.998||errorAngle==0)
...
void Estimator::optimizationwithLine()
{
    ...
    float pointInfo = f_m_cnt;
    float lineInfo = line_m_cnt;
    float pointAndLineInfo = pointInfo + lineInfo;
    float ratio = 0.5 * f_m_cnt / line_m_cnt / 400.;
    if (pointInfo > 0 && lineInfo > 0)
    {
        // ProjectionFactor::sqrt_info = lineInfo / pointAndLineInfo * FOCAL_LENGTH / 0.5 * Matrix2d::Identity();
        lineProjectionFactor::sqrt_info = pointInfo / pointAndLineInfo * FOCAL_LENGTH / ratio * Matrix2d::Identity();
    }
