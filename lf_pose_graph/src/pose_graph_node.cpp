...
void process()
{
    ...
    while (true)
    {
        ...
        if(tmp_t.toc()>1.0){
            ofstream foutC("/home/jiangdi/result_output/time/euroc/EPLF_VINS_WS/eplfvins_loopdetection.csv", ios::app);
            foutC << tmp_t.toc() << endl;
            foutC.close();
        }
...
{
    ros::init(argc, argv, "lf_pose_graph");
    ros::NodeHandle n("~");
    ...
    if (LOOP_CLOSURE)
    {
        ...
        std::string pkg_path = ros::package::getPath("lf_pose_graph");
        ...
        // fout.close();
        fsSettings.release();
        // std::ofstream foutC("/home/jiangdi/EPLF_WS/EPLF_VINS_WS/src/Trajactory/tum_fast_plvins_loop.txt", std::ios::out);
        // foutC.close();
        // std::ofstream foutC1("/home/jiangdi/EPLF_WS/EPLF_VINS_WS/src/Trajactory/evo_fast_plvins_loop.txt", std::ios::out);
        // foutC1.close();     
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/lfvins_estimator/imu_propagate", 2000, imu_forward_callback);
    ros::Subscriber sub_vio = n.subscribe("/lfvins_estimator/odometry", 2000, vio_callback);
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback);
    ros::Subscriber sub_pose = n.subscribe("/lfvins_estimator/keyframe_pose", 2000, pose_callback);
    ros::Subscriber sub_extrinsic = n.subscribe("/lfvins_estimator/extrinsic", 2000, extrinsic_callback);
    ros::Subscriber sub_point = n.subscribe("/lfvins_estimator/keyframe_point", 2000, point_callback);
    ros::Subscriber sub_relo_relative_pose = n.subscribe("/lfvins_estimator/relo_relative_pose", 2000, relo_relative_pose_callback);
