              ...
              const sensor_msgs::ImageConstPtr &img_msg)
{
  ...
  for (int i = 0; i < line_feature_msg->points.size(); i++)
  {
    ...
    // cv::Point2f endPoint = cv::Point2f(line_feature_msg->channels[5].values[i], line_feature_msg->channels[6].values[i]);
...
int main(int argc, char **argv)
{
  ...
  m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile("/YOUR_WS_PATH/EPLF-VINS/src/config/euroc/euroc_config_fix_extrinsic.yaml");
  K_ = m_camera->initUndistortRectifyMap(undist_map1_, undist_map2_);

  ros::init(argc, argv, "sync_control_node");
  ros::NodeHandle nh;

  message_filters::Subscriber<sensor_msgs::PointCloud> point_feature_sub(nh, "/lf_feature_tracker/feature", 1000);
  message_filters::Subscriber<sensor_msgs::PointCloud> line_feature_sub(nh, "/lf_linefeature_tracker/linefeature", 1000);
  message_filters::Subscriber<sensor_msgs::Image> image_sub(nh, "/cam0/image_raw", 1000);
  // message_filters::Subscriber<sensor_msgs::Image> image_sub(nh, "/mono_camera/image_raw", 1000);
